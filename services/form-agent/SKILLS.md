# SKILLS.md - form_agent Architecture & Coding Standards

This document outlines the specific domain knowledge, coding conventions, and architectural
patterns required for the `form_agent` Python project. Agents must adhere to
...
- **Synchronization Workflow:** When you need to add or change a configuration setting:
  ...
  2. Run `make generate-data-model` to sync `src/form_agent/data_model.py` and `.env.example`.
- **Prohibition:** NEVER define redundant `Settings` classes or load `.env` files manually using `dotenv` for core app logic.
  - **✅ Good:** `config = form_agent.get_env_config()`
...
- **Package Root Imports:** Always prefer importing core functions directly from the package root.
  - **✅ Good:** `from form_agent import get_env_config, get_logger`
  - **❌ Bad:** `from form_agent.core.logger import get_logger`
...
- **Module Exports:** Ensure `src/form_agent/__init__.py` properly exports all new public components via `__all__ = [...]`.
