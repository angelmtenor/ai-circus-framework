...
def regression_artifacts() -> ModelArtifacts:
    """A tiny real regression pipeline+TreeExplainer trained on synthetic data."""
    rng = np.random.default_rng(0)
    n = 60
    df = pd.DataFrame({
        "numeric_feature": rng.normal(size=n),
    ...
    target = df["numeric_feature"] * 2 + rng.normal(scale=0.1, size=n)
    ...
    x_transformed = pipeline.named_steps["preprocessor"].transform(df)
    explainer = shap.TreeExplainer(
        pipeline.named_steps["model"], data=x_transformed, feature_perturbation="interventional"
    )
...
def test_predict_returns_raw_value_for_regression(regression_artifacts: ModelArtifacts) -> None:
    """For a regression scenario, predict() calls pipeline.predict() (not predict_proba)
    and returns the raw value, not a 0-1 probability.
    """
    records = pd.DataFrame([{"numeric_feature": 3.0, "category_feature": "A"}])
