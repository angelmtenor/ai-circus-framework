# SKILLS.md - etl_tabular Architecture & Coding Standards

This document outlines the specific domain knowledge, coding conventions, and architectural
patterns required for the `etl_tabular` Python project. Agents must adhere to
...
- **Synchronization Workflow:** When you need to add or change a configuration setting:
  ...
  2. Run `make generate-data-model` to sync `src/etl_tabular/data_model.py` and `.env.example`.
- **Prohibition:** NEVER define redundant `Settings` classes or load `.env` files manually using `dotenv` for core app logic.
  - **✅ Good:** `config = etl_tabular.get_env_config()`
...
- **Package Root Imports:** Always prefer importing core functions directly from the package root.
  - **✅ Good:** `from etl_tabular import get_env_config, get_logger`
  - **❌ Bad:** `from etl_tabular.core.logger import get_logger`
...
- **Module Exports:** Ensure `src/etl_tabular/__init__.py` properly exports all new public components via `__all__ = [...]`.
