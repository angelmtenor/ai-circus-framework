# SKILLS.md - data_platform_manager Architecture & Coding Standards

This document outlines the specific domain knowledge, coding conventions, and architectural
patterns required for the `data_platform_manager` Python project. Agents must adhere to
...
- **Synchronization Workflow:** When you need to add or change a configuration setting:
  ...
  2. Run `make generate-data-model` to sync `src/data_platform_manager/data_model.py` and `.env.example`.
- **Prohibition:** NEVER define redundant `Settings` classes or load `.env` files manually using `dotenv` for core app logic.
  - **✅ Good:** `config = data_platform_manager.get_env_config()`
...
- **Package Root Imports:** Always prefer importing core functions directly from the package root.
  - **✅ Good:** `from data_platform_manager import get_env_config, get_logger`
  - **❌ Bad:** `from data_platform_manager.core.logger import get_logger`
