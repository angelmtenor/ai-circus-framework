"""Tests for dl-inference's core/identity.py — a thin FastAPI wrapper around
...
import dl_inference.core.identity as identity_module
...
def test_admin_api_key_is_unwrapped_from_secretstr(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    identity_module.resolve_identity(scenario_slug="symptom_triage", authorization="Bearer ai-circus-2026")

    settings = captured["settings"]
    assert isinstance(settings, AuthSettingsAdapter)
    assert settings.ADMIN_API_KEY == "ai-circus-2026"
    assert captured["scenario_slug"] == "symptom_triage"
...
def test_token_validation_error_becomes_401(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        identity_module.resolve_identity(scenario_slug="symptom_triage", authorization=None)
...
def test_entitlement_denied_becomes_403(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        identity_module.resolve_identity(scenario_slug="symptom_triage", authorization="Bearer good-token")
...
def test_entitled_caller_resolves_successfully(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **_kw: Identity(subject="user-1", org_id="org-1", roles=frozenset({"scenario:symptom_triage"})),
    )

    identity = identity_module.resolve_identity(scenario_slug="symptom_triage", authorization="Bearer good-token")
