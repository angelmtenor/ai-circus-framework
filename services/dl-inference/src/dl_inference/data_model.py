...
class EnvConfig(BaseSettings):
    ...
    SCENARIOS: str = Field(
        description="Comma-separated deep_learning scenario slugs this instance serves; empty/unset = every scenario"
    )
    ...
    SHARED_MODEL_ORG_ID: str = Field(
        description="Org id every tenant's model falls back to until it has its own (matches dl-training's ORG_ID)"
    )
    ...
    ORT_THREADS: str = Field(
        description="onnxruntime intra-op threads per inference session (keeps the pod's CPU use bounded)"
    )
