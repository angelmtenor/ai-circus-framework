# SKILLS.md - assistant Architecture & Coding Standards

This document outlines the specific domain knowledge, coding conventions, and architectural
patterns required for the `assistant` Python project. Agents must adhere to
...
- **Synchronization Workflow:** When you need to add or change a configuration setting:
  ...
  2. Run `make generate-data-model` to sync `src/assistant/data_model.py` and `.env.example`.
- **Prohibition:** NEVER define redundant `Settings` classes or load `.env` files manually using `dotenv` for core app logic.
  - **✅ Good:** `config = assistant.get_env_config()`
...
- **Package Root Imports:** Always prefer importing core functions directly from the package root.
  - **✅ Good:** `from assistant import get_env_config, get_logger`
  - **❌ Bad:** `from assistant.core.logger import get_logger`
...
- **Module Exports:** Ensure `src/assistant/__init__.py` properly exports all new public components via `__all__ = [...]`.
