# SKILLS.md - agui_voice Architecture & Coding Standards

This document outlines the specific domain knowledge, coding conventions, and architectural
patterns required for the `agui_voice` Python project. Agents must adhere to
...
- **Synchronization Workflow:** When you need to add or change a configuration setting:
  ...
  2. Run `make generate-data-model` to sync `src/agui_voice/data_model.py` and `.env.example`.
- **Prohibition:** NEVER define redundant `Settings` classes or load `.env` files manually using `dotenv` for core app logic.
  - **✅ Good:** `config = agui_voice.get_env_config()`
...
- **Package Root Imports:** Always prefer importing core functions directly from the package root.
  - **✅ Good:** `from agui_voice import get_env_config, get_logger`
  - **❌ Bad:** `from agui_voice.core.logger import get_logger`
...
- **Module Exports:** Ensure `src/agui_voice/__init__.py` properly exports all new public components via `__all__ = [...]`.
